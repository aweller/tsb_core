...
#         print self.chrompos, all(filterset), filterset, self.row
...
# #             try:                
#                 
#             old_alt = int(self.old.FAO)
#             old_total = int(self.old.FAO) + int(self.old.FRO)
#             new_alt = int(self.new.FAO)
#             new_total = int(self.new.FAO) + int(self.new.FRO)                
#             
#             oddsratio, pvalue = scipy.stats.fisher_exact([[old_alt, old_total], [new_alt, new_total]])
#             
#             self.pvalue = pvalue
    ...
    def fetch_output_row(self, common_fields_to_print, fields_to_print, filter_code):
        ...
        #common_fields = [self.chrompos, self.ref]
...
class ComparisonDataset:
    ...
    def create_header_row(self, fields_to_print, comp_name):
        ...
        if comp_name[0] == "F": # the special case of the F3_24023_v1_CHP1_IR14 files
            sample = "_".join(comp_name.split("_")[:2])
#         common_fieldno = len(fields_to_print[0])
#         unique_fieldno = len(fields_to_print[1])
             
        header = [sample, ]
        
#         header.extend(["Common\t" * common_fieldno])
#         header.extend([(oldname+"\t") * unique_fieldno])
#         header.extend([(newname+"\t") * unique_fieldno])
#         header.extend("\n")        
